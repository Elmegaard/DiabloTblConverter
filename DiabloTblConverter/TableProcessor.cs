...
public class TableProcessor
{
    ...
    static ushort getCRC(byte[] stringData, int size)
    {
        ushort[] CRCTable = {
            0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7, 0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
        0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6, 0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
        ...
        {
            byte charvalue = (byte)(stringData[i] ^ (CRCValue >> 8));
            ushort temp = (ushort)((CRCValue & 0xFF) << 8);
            CRCValue = (ushort)(CRCTable[charvalue] ^ temp);
        }
        return CRCValue;
    ...
    public static uint getHashValue(string key, int hashTableSize)
    {
        char currentChar;
    ...
    class TableEntryData
    {
        ...
        public string EncodedKey => _enc1252.GetString(KeyBytes);
    ...
    public static void WriteTablesFile(string outputPath, string json)
    {
        ...
        //var indices = new ushort[entriesCount];
        //var nodes = new TblHashNode[entriesCount];
        var collisionsDetected = new bool[entriesCount];
        //var keys = new string[entriesCount];
        //var values = new string[entriesCount];
        ...
        using (var tblFile = new FileStream(outputPath, FileMode.Create))
        {
            using (var writer = new BinaryWriter(tblFile))
            {
                ...
                var dataBytes = new byte[fileSize - dataStartOffset];
                writer.BaseStream.Read(dataBytes);
